...
func ContractLiquidityWithdrawFund() *cobra.Command {
	cmd := &cobra.Command{
		Use: "contract-liquidity-withdraw-fund",
		Long: `Liquidity withdraw funds if found any security risk.
Usage:
contract-liquidity-withdraw-fund --chain [Chain] --contract-hash [Liquidity pool address] --tokens [List of tokens] --new-owner [New owner address] --index [Index of this message]
...
`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewLiquidityWithdrawFundMsg(clientCtx.GetFromAddress().String(), chain, liquidityAddress, tokens, newOwner, index)
	...
	cmd.Flags().String(flags.Tokens, "", "array of tokens")
