...
type TestnetDockerExecutor struct {
	ropstenUrl         string
	ropstenBlockNumber uint64
}

// get cmd to initialize all files for tendermint localnet and application
func TestnetDockerCmd(mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "testnet-docker",
...
Example:
  For multiple nodes (running with docker):
    ./sisu testnet-docker --v 2 --output-dir ./output --ropsten-url https://ropsten.infura.io/v3/[YOUR_TOKEN]
  `,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			exe.ropstenUrl, _ = cmd.Flags().GetString(flagRopstenUrl)
			algo, _ := cmd.Flags().GetString(flags.FlagKeyAlgorithm)

			// Clean up
			exe.cleanData(outputDir)
			...
			chainIds := []*big.Int{libchain.GetChainIntFromId("eth-sisu-local"), libchain.GetChainIntFromId("eth-ropsten")}
			dockerConfig := exe.getDockerConfig([]string{"192.168.10.2", "192.168.10.3"}, chainIds, "192.168.10.4", ips)
			...
			for i := range ips {
				...
				nodeConfig := exe.getNodeSettings(chainID, keyringBackend, i, mysqlIp, ips)
				nodeConfigs[i] = nodeConfig

				exe.generateEyesToml(i, dockerConfig, dir)
			}

			exe.generateDockerCompose(filepath.Join(outputDir, "docker-compose.yml"), ips, dockerConfig)
			...
			peerIds, err := exe.getPeerIds(len(ips), valPubKeys)
			if err != nil {
				panic(err)
			}
			...
			for i := range ips {
				dir := filepath.Join(outputDir, fmt.Sprintf("node%d", i))
				exe.generateHeartToml(i, dir, dockerConfig, peerIds)
			}
...
func (exe *TestnetDockerExecutor) cleanData(outputDir string) {
...
func (exe *TestnetDockerExecutor) getBlockNumber() {
	log.Info("Getting Ropsten block height...")
	client, err := ethclient.Dial(exe.ropstenUrl)
	if err != nil {
		panic(err)
	}
	...
	exe.ropstenBlockNumber = blockNumber
}

func (exe *TestnetDockerExecutor) getDockerConfig(ganacheIps []string, chainIds []*big.Int, mysqlIp string, ips []string) DockerNodeConfig {
...
func (exe *TestnetDockerExecutor) getNodeSettings(chainID, keyringBackend string, index int, mysqlIp string, ips []string) config.Config {
...
func (exe *TestnetDockerExecutor) generateDockerCompose(outputPath string, ips []string, dockerConfig DockerNodeConfig) {
...
func (exe *TestnetDockerExecutor) generateEyesToml(index int, dockerConfig DockerNodeConfig, dir string) {
	data := struct {
		Index                int
		...
		RopstenUrl:           exe.ropstenUrl,
		RopstenStartingBlock: exe.ropstenBlockNumber,
	}
...
[chains.eth-ropsten]
  chain = "eth-ropsten"
  block_time = 6000
  starting_block = {{ .RopstenStartingBlock }}
  rpc_url = "{{ .RopstenUrl }}"
`
...
func (exe *TestnetDockerExecutor) getPeerIds(n int, pubKeys []cryptotypes.PubKey) ([]string, error) {
	ids := make([]string, n)
...
func (exe *TestnetDockerExecutor) generateHeartToml(index int, dir string, dockerConfig DockerNodeConfig, peerIds []string) {
