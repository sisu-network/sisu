...
func ResumeContractCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use: "resume-contract",
		Long: `Resume a contract.
Usage:
resume-contract --chain [Chain] --name [ContractName] --index [Resume Command Index]

Example:
./sisu resume-contract --chain ganache1 --name erc20gateway --index 0
`,
		RunE: func(cmd *cobra.Command, args []string) error {
			chain, _ := cmd.Flags().GetString(flags.Chain)
			name, _ := cmd.Flags().GetString(flags.Name)
			...
			msg := types.NewResumeContractMsg(clientCtx.GetFromAddress().String(), chain, hash, index)
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
