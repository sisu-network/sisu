...
type TxOutputProducer interface {
	// GetTxOut generates a TxOut message by loop over all transfers until reaching a failure
	// transfer. An error will be returned if can not generate txout or the first transfer is
	// failed.
	GetTxOut(ctx sdk.Context, chain string, transfers []*types.TransferDetails) (*types.TxOut, error)
}
...
func (p *DefaultTxOutputProducer) GetTxOut(ctx sdk.Context, chain string,
